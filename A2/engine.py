...
class ExecutionEngine:
    ...
    def run(self, strategies: List[Strategy]):
        ...
        self.load_data("data/market_data.csv")
        ...
        for tick in self._market_data:     
            for strategy in strategies:
                if strategy._symbol != tick.symbol:
                    continue
                try:
                    signals = strategy.generate_signals(tick, tick.daily_volume * 0.075)
                    strategy_class = strategy.__class__.__name__
                    symbol = strategy._symbol
    ...
    def _execute_order_direct(self, order: Order, signal_side: str, strategy: Strategy):
        ...
        order.status = "FILLED"
        logger.info(f"Executed {signal_side}: {order.symbol} {order.quantity}@{order.price:.2f} | Strategy: {strategy_class}_{symbol} | Capital: ${strategy.remaining_capital:.2f}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--error-demo":
        ...
        snp500_symbols = pd.read_csv('data/sp500_symbols.csv')['Symbol'].tolist()
        StrategyClasses = ['BenchmarkStrategy', 
                           #'MACDStrategy', 'RSIStrategy',
                            #'MovingAverageStrategy', 'VolatilityBreakoutStrategy'
                           ]
       
        # Instantiate strategies: for each symbol create one instance of each strategy
        strat_list = []
        ...
        for sym in snp500_symbols:
            ...
            v = VolatilityBreakoutStrategy(symbol=sym, capital=0)
            strat_list.append(v)

            # # MACDStrategy(capital)
            m = MACDStrategy(symbol=sym, capital=0)
            strat_list.append(m)

            # # RSIStrategy(capital)
            r = RSIStrategy(symbol=sym, capital=0)
            strat_list.append(r)

            # # MovingAverageStrategy(symbol, capital, ...)
            ma = MovingAverageStrategy(symbol=sym, capital=0)
            strat_list.append(ma)

            # BenchmarkStrategy(capital)
            b = BenchmarkStrategy(symbol=sym, capital=0)
            strat_list.append(b)
